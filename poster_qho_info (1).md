...
El oscilador armónico es uno de los sistemas más fundamentales en mecánica cuántica, con aplicaciones en óptica cuántica, física de estado sólido, y teoría cuántica de campos.

### **Hamiltoniano**
$$\hat{H} = \frac{\hat{p}^2}{2m} + \frac{1}{2}m\omega^2\hat{x}^2$$

**Formulación en operadores de creación/aniquilación:**
...
$$\hat{H}|n\rangle = \hbar\omega(n + 1/2)|n\rangle, \quad n = 0, 1, 2, ...$$

Construidos mediante:
$$|n\rangle = \frac{(\hat{a}^\dagger)^n}{\sqrt{n!}}|0\rangle$$

### **Operadores de Cuadratura (Adimensionales)**
...
donde:
$$\Delta X = \sqrt{\langle \hat{X}^2 \rangle - \langle \hat{X} \rangle^2}, \quad \Delta P = \sqrt{\langle \hat{P}^2 \rangle - \langle \hat{P} \rangle^2}$$

---

### **Representación en Espacio de Fase**

A diferencia del caso clásico donde el estado está completamente determinado por el punto $(x, p)$, en mecánica cuántica el estado cuántico requiere una distribución de probabilidad.

### **Función de Wigner**
Representación cuasi-probabilística del estado cuántico en espacio de fase:

$$W(X, P) = \frac{1}{\pi}\int_{-\infty}^{\infty} \langle X - y|\hat{\rho}|X + y\rangle e^{2iPy} dy$$

**Propiedades:**
- $\int\int W(X,P) dX dP = 1$ (normalización)
- Las marginales dan las distribuciones correctas:
  - $\int W(X,P) dP = |\psi(X)|^2$
  - $\int W(X,P) dX = |\tilde{\psi}(P)|^2$
- ⚠️ **Puede ser negativa** (signo de no-clasicalidad)

### **Matriz de Covarianza**
Para estados gaussianos, toda la información está contenida en:

$$\Sigma = \begin{pmatrix} \langle \Delta X^2 \rangle & \langle \Delta X \Delta P + \Delta P \Delta X \rangle/2 \\ \langle \Delta X \Delta P + \Delta P \Delta X \rangle/2 & \langle \Delta P^2 \rangle \end{pmatrix}$$

**Representación geométrica:**
La matriz $\Sigma$ define una **elipse de incerteza** en el espacio de fase:
- Los autovalores $\lambda_{1,2}$ determinan los semiejes
- Los autovectores determinan la orientación
- El área es $A = \pi\sqrt{\det(\Sigma)}$

---

### **Diferencias Cuántico-Clásicas Fundamentales**

| Aspecto | Clásico | Cuántico |
|---------|---------|----------|
| **Estado** | Punto $(x, p)$ | Vector $\|\psi\rangle$ o matriz $\hat{\rho}$ |
| **Evolución** | Determinista | Probabilística |
| **Incerteza** | ❌ Nula (mediciones perfectas) | ✅ Fundamental (Heisenberg) |
| **Espacio de fase** | Punto | Distribución (elipse) |
| **Área mínima** | 0 | $\pi\hbar/2$ (estados puros) |
| **Interferencia** | ❌ No existe | ✅ Superposición de estados |
...
Desplaza el estado en el espacio de fase:
$$\hat{D}^\dagger(\alpha)\hat{a}\hat{D}(\alpha) = \hat{a} + \alpha$$

**2. Operador de Squeezing**
$$\hat{S}(r,\theta) = \exp\left[\frac{r}{2}(e^{-2i\theta}\hat{a}^2 - e^{2i\theta}\hat{a}^{\dagger 2})\right]$$

Comprime una cuadratura y expande la conjugada, manteniendo el área constante.

**3. Operador Rotación**
$$\hat{R}(\phi) = e^{i\phi\hat{a}^\dagger\hat{a}}$$

Rota el estado en el espacio de fase por ángulo $\phi$.
...
Los estados evolucionan mediante **rotación** en el espacio de fase con frecuencia $\omega$.

---

## 🎯 OBJETIVO DE LAS SIMULACIONES

Visualizar las diferencias entre:
1. **Estados cuánticos puros** con diferentes propiedades estadísticas
2. **Comportamiento clásico** del oscilador (límite $\hbar \to 0$)

Enfoque: **Espacio de fase $(X, P)$** con representación visual de:
- Valores esperados $\langle X \rangle$, $\langle P \rangle$ (trayectoria)
- Incertezas $\Delta X$, $\Delta P$ (elipse)
- Evolución temporal en tiempo real
...
### **Función de Wigner**
$$W_\alpha(X, P) = \frac{2}{\pi}\exp\left[-2(X - \sqrt{2}\text{Re}[\alpha])^2 - 2(P - \sqrt{2}\text{Im}[\alpha])^2\right]$$

Gaussiana positiva centrada en $(\sqrt{2}\text{Re}[\alpha], \sqrt{2}\text{Im}[\alpha])$
...
✓ Distribución gaussiana sin negatividad
...
### **Función de Wigner**
$$W(X, P) = \frac{2}{\pi}\exp[-2(X_r^2/e^{2r} + P_r^2/e^{-2r})]$$

Gaussiana positiva pero **elíptica** (comprimida en una dirección)
...
Oscilan con frecuencias $\omega(n-m)$ → **Interferencia cuántica**

### **Función de Wigner**
Para superposiciones, la función de Wigner puede tener **valores negativos** → signo inequívoco de no-clasicalidad.
...
✓ Posible negatividad en Wigner (no-clasicalidad)
...
✓ Límite $\hbar \to 0$ de los estados cuánticos
...
| Estado | Valores ⟨X⟩, ⟨P⟩ | Incertezas ΔX, ΔP | Elipse | No-clasicalidad |
|--------|------------------|-------------------|--------|-----------------|
| **Coherente** | Circular | Constantes (1/√2) | Círculo rígido que rota | Baja (Wigner > 0) |
| **Comprimido** | Circular | Oscilan entre ejes | Elipse rígida que rota | Media (Wigner > 0) |
| **Superposición** | Compleja | Variables dinámicas | Se DEFORMA continuamente | **Alta (Wigner < 0)** |
| **Clásico** | Circular | ❌ Sin incerteza | Punto (sin extensión) | Nula |
