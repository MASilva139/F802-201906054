...
RESULTADOS_DIR = Path('Proyecto/Final/resultados')
...
GENERAR_ANIMACION_AUTO = True  # True para generar automáticamente
...
class AnalizadorQHO:
    ...
    def _detectar_tipo(self):
        ...
        if 'Sigma_XX' in columnas and 'Sigma_PP' in columnas:
            # Verificar si hay squeezing significativo
            if len(self.df) > 0:
                ratio = self.df['Sigma_XX'].mean() / (self.df['Sigma_PP'].mean() + 1e-10)
                if abs(ratio - 1.0) > 0.1:  # Anisotropía significativa
                    return 'estado_comprimido'
    ...
    def _verificar_area_liouville(self):
        ...
        # Para estado coherente, área ≈ π/2 (una desviación estándar)
        # Para squeezed, también debería conservarse
        area_teorica = np.pi / 2  # Para 1σ
        
        if area_std / area_media < 0.01:
            print(f"  ✅ ÁREA CONSERVADA (variación < 1%)")
        else:
            print(f"  ⚠️  Variación: {area_std/area_media*100:.2f}%")
    ...
    def _verificar_energia(self):
        ...
        # Para sistemas con drive, la energía puede cambiar
        if self.tipo in ['green_split_operator', 'green_analitico']:
    ...
    def analizar_generico(self):
        ...
        # Plot de las primeras columnas numéricas
    ...
    def guardar_figuras(self):
        ...
        print(f"💾 Figura principal: {filename}")
        
        # FFT
        if hasattr(self, 'fig_fft'):
            fft_file = FFT_DIR / f'{self.nombre}_fft.png'
            self.fig_fft.savefig(fft_file, dpi=300, bbox_inches='tight')
            print(f"💾 FFT: {fft_file}")
        
        # Ajuste
        if hasattr(self, 'fig_ajuste'):
            ajuste_file = AJUSTES_DIR / f'{self.nombre}_ajuste.png'
            self.fig_ajuste.savefig(ajuste_file, dpi=300, bbox_inches='tight')
            print(f"💾 Ajuste: {ajuste_file}")
...
class ComparadorSimulaciones:
    ...
    def __init__(self, archivos):
        """
        Inicializa el comparador con lista de archivos CSV
        
        Args:
            archivos: Lista de rutas a archivos CSV
        """
    ...
    def _get_color(self, tipo):
        """Obtiene color consistente para cada tipo"""
        return COLORES_SIMULACION.get(tipo, 'gray')
    
    def _get_nombre(self, tipo):
        """Obtiene nombre legible para cada tipo"""
    ...
    def comparar_con_clasico(self):
        ...
        # 4. Incertezas cuánticas (solo cuánticos)
        ax4 = plt.subplot(2, 3, 4)
        ...
        # Interpolar trayectoria clásica para comparar
        ...
        for analizador in cuanticos:
            ...
            # Interpolar clásico a tiempos cuánticos
    ...
    def comparar_energias(self):
        """Compara conservación de energía entre todas las simulaciones"""
    ...
    def comparar_green_functions(self):
        """Compara específicamente las dos implementaciones de Green's function"""
    ...
    def tabla_comparativa(self):
        """Genera tabla comparativa con estadísticas de todas las simulaciones"""
        print("\n📋 TABLA COMPARATIVA")
        print("=" * 100)
...
def analizar_directorio(directorio='Proyecto/Final/CSV'):
    ...
    # Comparación si hay múltiples archivos
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--comparar':
            # Modo comparación
            if len(sys.argv) < 4:
                print("❌ Uso: python analisis.py --comparar archivo1.csv archivo2.csv ...")
                sys.exit(1)
        ...
        # Sin argumentos: analizar directorio actual
